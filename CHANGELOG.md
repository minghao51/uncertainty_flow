...
- Replaced the legacy `BenchmarkConfig`, `BenchmarkFlow`, `BenchmarkRunner`, result/sink,
  and translator APIs with `RunRequest`, `PipelineRunResult`, typed provider registries,
  and verified medallion artifacts.
- Removed `benchmarks/run_benchmarks.py`; use `uf benchmark` or the pipeline coordinators.
